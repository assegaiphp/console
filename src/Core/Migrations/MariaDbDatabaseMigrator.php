...
/**
 * Class MariaDbDatabaseMigrator. This class is a migrator for MariaDB databases.
 *
 * @package Assegai\Console\Core\Migrations
 */
class MariaDbDatabaseMigrator extends MariaDbDatabase implements MigratorInterface
{
  /**
   * @inheritDoc
  ...
  public function create(string $name): string|false
  {
    ...
    $path = Path::join($this->getMigrationsDirectoryPath(), DatabaseType::MARIADB->value, $this->name, $directoryName);
  ...
  {
    return Path::join(getcwd() ?: '', 'migrations', DatabaseType::MARIADB->value, $this->name);
  }
