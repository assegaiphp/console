...
class MsSqlDatabaseMigrator extends MsSqlDatabase implements MigratorInterface
{
  ...
  public function create(string $name): string|false
  {
    ...
    $path = Path::join($this->getMigrationsDirectoryPath(), DatabaseType::MSSQL->value, $this->name, $directoryName);
  ...
  {
    $migrationsTableName = self::getMigrationsTableName();
    $query = "SELECT TOP 1 migration FROM  ORDER BY ran_at DESC";
  ...
  {
    return Path::join(getcwd() ?: '', 'migrations', DatabaseType::MSSQL->value, $this->name);
  }
